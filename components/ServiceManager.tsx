...
const ServiceNoteModal: React.FC<{ 
  ...
  const handleDownloadPDF = async () => {
    ...
    if (typeof html2pdf === 'undefined') {
      alert('Carregando sistema de PDF... Tente novamente em 2 segundos.');
      return;
    }
  ...
  const handleShareWhatsApp = () => {
    ...
    const materialList = service.materials.map(sm => {
      ...
      return `• ${mat?.name} (${sm.quantity} UN)`;
    }).join('\n');

    const message = `*RM ELÉTRICA & SOLUÇÕES*%0A` +
      `--------------------------------%0A` +
      `*Relatório de Serviço:* #${service.id.slice(-6).toUpperCase()}%0A` +
      `*Cliente:* ${customer.name}%0A` +
      `*Data:* ${new Date(service.date).toLocaleDateString('pt-BR')}%0A%0A` +
      `*Descrição:* ${service.description}%0A%0A` +
      `*Materiais:* %0A${materialList || 'Nenhum material cadastrado'}%0A%0A` +
      `*Valor Total:* R$ ${service.totalValue.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}%0A` +
      `*Pagamento:* ${service.paymentMethod}%0A` +
      `--------------------------------%0A` +
      `*Técnico Responsável:* Renan Morais%0A` +
      `*WhatsApp:* (14) 99179-8868%0A%0A` +
      `_Obrigado pela confiança! Segue o arquivo PDF em anexo._`;
    ...
    <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-md z-[300] flex items-center justify-center p-4 md:p-10 overflow-hidden">
      {isGenerating && (
        <div className="fixed inset-0 bg-indigo-900/60 z-[310] flex flex-col items-center justify-center text-white backdrop-blur-sm">
          ...
          <p className="font-black uppercase tracking-widest">Processando Documento...</p>
        </div>
      )}
      ...
      <div className="bg-slate-100 w-full max-w-5xl h-full flex flex-col rounded-[2rem] shadow-2xl overflow-hidden border border-white/20 animate-in zoom-in-95 duration-300">
        ...
        <div className="p-4 bg-white border-b flex flex-wrap gap-2 justify-between items-center shadow-sm shrink-0">
          <button onClick={onClose} className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-xl font-bold transition-all text-sm">
            ✕ Fechar
          </button>
          ...
          <div className="flex gap-2">
            <button 
              ...
              className="bg-emerald-500 hover:bg-emerald-600 text-white px-4 md:px-6 py-2.5 rounded-xl font-black text-xs md:text-sm shadow-lg shadow-emerald-100 flex items-center gap-2 transition-all"
            >
              <span>📱</span> WHATSAPP
            </button>
            <button 
              onClick={handleDownloadPDF}
              className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 md:px-6 py-2.5 rounded-xl font-black text-xs md:text-sm shadow-xl shadow-indigo-100 flex items-center gap-2 transition-all"
            >
          ...
          <div id="professional-invoice-template" className="bg-white w-full max-w-[210mm] mx-auto p-12 md:p-16 shadow-2xl rounded-sm text-slate-800" style={{ minHeight: '297mm' }}>
            ...
            <div className="flex justify-between items-start border-b-8 border-indigo-600 pb-10 mb-12">
              <div className="space-y-1">
                <h1 className="text-4xl font-black text-slate-900 tracking-tighter leading-none uppercase">RM ELÉTRICA</h1>
                <h2 className="text-lg font-bold text-indigo-600 uppercase tracking-widest">Serviços Profissionais</h2>
                <div className="mt-8 text-xs text-slate-600 font-bold space-y-1 uppercase tracking-tight">
                  <p className="flex items-center gap-2">Técnico: Renan Morais</p>
                  <p className="flex items-center gap-2">Contato: (14) 99179-8868</p>
                </div>
              </div>
              <div className="text-right space-y-2">
                <div className="bg-slate-900 text-white px-6 py-4 rounded-xl inline-block shadow-lg">
                  <p className="text-[10px] font-black uppercase opacity-60 tracking-widest">Ordem de Serviço</p>
                  <p className="text-2xl font-black"># {service.id.slice(-6).toUpperCase()}</p>
                </div>
                <p className="text-xs font-black text-slate-400 uppercase">{new Date(service.date).toLocaleDateString('pt-BR', { day: '2-digit', month: 'long', year: 'numeric' })}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-12 mb-12">
              <div className="space-y-4">
                <h4 className="text-[10px] font-black text-indigo-600 uppercase tracking-widest border-b-2 border-indigo-50 pb-2">Destinatário</h4>
                <div className="space-y-1">
                  <p className="text-xl font-black text-slate-800 leading-tight">{customer?.name || 'Cliente Particular'}</p>
                  <p className="text-sm font-semibold text-slate-500">{customer?.street}, {customer?.neighborhood}</p>
                  <p className="text-sm font-semibold text-slate-500">{customer?.city} - {customer?.state} | CEP: {customer?.cep}</p>
                  <p className="text-sm font-black text-indigo-600 mt-2">WhatsApp: {customer?.phone}</p>
                </div>
              </div>
              <div className="space-y-4 text-right">
                <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest border-b-2 border-slate-50 pb-2">Pagamento</h4>
                <div className="space-y-1">
                  ...
                  {service.paymentMethod === PaymentMethod.CREDIT_CARD && (
                    <p className="text-sm font-bold text-indigo-500">Parcelado em {service.installments}x</p>
                  )}
                  <div className={`mt-3 inline-block px-4 py-1.5 rounded-full text-[9px] font-black uppercase shadow-sm ${service.status === ServiceStatus.COMPLETED ? 'bg-emerald-600 text-white' : 'bg-amber-500 text-white'}`}>
                    {service.status}
                  </div>
                </div>
              </div>
            ...
            <div className="mb-12">
              ...
              <div className="bg-slate-50 p-10 rounded-[2.5rem] border-l-[12px] border-indigo-600 shadow-inner">
            ...
            {/* Itens e Valores */}
            <div className="mb-16">
              <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">Detalhamento de Investimento</h4>
              <div className="overflow-hidden rounded-[2rem] border border-slate-200 shadow-lg">
                <table className="w-full">
                  <thead>
                    <tr className="bg-slate-900 text-white text-[10px] font-black uppercase tracking-wider">
                      <th className="px-8 py-6 text-left">Descrição dos Serviços e Itens</th>
                  ...
                  <tbody className="divide-y divide-slate-100 text-sm">
                    <tr>
                      <td className="px-8 py-8">
                        <p className="font-black text-slate-900 text-base">Mão de Obra Técnica Especializada</p>
                        <p className="text-[10px] text-slate-400 uppercase tracking-widest mt-1">Garantia total de execução profissional</p>
                      </td>
                      <td className="px-8 py-8 text-center font-bold">1</td>
                      <td className="px-8 py-8 text-right font-black text-slate-900">R$ {service.serviceValue.toFixed(2)}</td>
                    </tr>
                      ...
                      return (
                        <tr key={idx} className="bg-slate-50/50">
                          <td className="px-8 py-6 text-slate-700">
                             ...
                             <p className="text-[10px] uppercase opacity-60">Material de instalação</p>
                          </td>
                          <td className="px-8 py-6 text-center font-bold">{sm.quantity}</td>
                          <td className="px-8 py-6 text-right font-bold text-slate-900">R$ {(sm.quantity * (mat?.sellingPrice || 0)).toFixed(2)}</td>
                        </tr>
                      );
                    })}
                  </tbody>
                  <tfoot className="bg-slate-900 text-white">
                    {service.discount > 0 && (
                      <tr className="bg-rose-600">
                        <td colSpan={2} className="px-8 py-4 text-right text-[10px] font-black uppercase tracking-widest">Bonificação Aplicada</td>
                        <td className="px-8 py-4 text-right font-black">- R$ {service.discount.toFixed(2)}</td>
                      </tr>
                    )}
                    <tr>
                      <td colSpan={2} className="px-8 py-10 text-left text-2xl font-black uppercase tracking-tighter">Valor Total do Atendimento</td>
                      <td className="px-8 py-10 text-right text-4xl font-black tracking-tighter text-indigo-400">
            ...
            {/* Assinaturas */}
            <div className="mt-24 grid grid-cols-2 gap-24 px-12 pt-12 border-t-2 border-dashed border-slate-200">
              <div className="text-center space-y-4">
                <div className="border-b-4 border-slate-900 w-full h-12"></div>
                <p className="text-[10px] font-black text-slate-900 uppercase tracking-[0.3em]">Renan Morais - RM Elétrica</p>
              </div>
              <div className="text-center space-y-4">
                <div className="border-b-4 border-slate-900 w-full h-12"></div>
                <p className="text-[10px] font-black text-slate-900 uppercase tracking-[0.3em]">{customer?.name || 'Assinatura do Cliente'}</p>
              </div>
            </div>

            <div className="mt-32 text-center">
               <p className="text-[9px] text-slate-300 font-black uppercase tracking-[0.6em]">Documento Oficial RM Elétrica & Soluções</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500 text-sm">Registro técnico e financeiro.</p>
        </div>
        <button 
          onClick={() => {
            if (isFormOpen) {
        ...
        >
          {isFormOpen ? 'Cancelar' : '➕ Novo Atendimento'}
        </button>
      </div>
      ...
      {isFormOpen && (
        <form onSubmit={handleAddService} className="bg-white p-6 md:p-10 rounded-[2.5rem] border-2 border-indigo-100 shadow-2xl space-y-10 animate-in slide-in-from-top-4 duration-300">
          <div className="flex items-center gap-4">
             <div className="bg-indigo-600 text-white p-4 rounded-2xl text-2xl shadow-lg">🛠️</div>
             <div>
                <h3 className="font-black text-slate-800 uppercase tracking-tighter text-xl">Lançamento Profissional</h3>
                <p className="text-xs text-slate-400 font-bold uppercase tracking-[0.2em]">Configuração de Ordem de Serviço</p>
             </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <div className="flex flex-col gap-2">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-1">Cliente Solicitante</label>
            ...
            <div className="md:col-span-3 bg-indigo-50/50 p-6 rounded-3xl border border-indigo-100">
              <label className="block text-[10px] font-black text-indigo-400 uppercase tracking-widest mb-2">Sugestões do Catálogo</label>
            ...
            <div className="md:col-span-3">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Relatório de Atividades</label>
              <textarea required value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} className="w-full p-6 rounded-3xl border-2 border-slate-100 bg-slate-50 focus:border-indigo-500 font-medium h-40 transition-all outline-none" placeholder="O que foi executado?" />
            </div>
            ...
            <div className="md:col-span-3 border-t-2 border-indigo-50 pt-10">
              <h4 className="text-sm font-black text-slate-800 uppercase tracking-tighter mb-6 flex items-center gap-2">
                <span className="bg-slate-900 text-white p-2 rounded-lg text-xs">📦</span> Materiais Aplicados
              </h4>
            ...
            <div className="flex gap-4">
              <div className="flex-1">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Pagamento</label>
          ...
          <div className="bg-slate-900 text-white p-10 rounded-[3rem] shadow-2xl relative overflow-hidden">
            <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-6">
              <div>
                 <p className="text-[10px] opacity-40 uppercase font-black tracking-[0.4em] mb-2">Investimento Estimado</p>
                 <h4 className="text-xl font-bold">Total do Atendimento</h4>
              </div>
          ...
          <button type="submit" className="w-full bg-indigo-600 text-white p-8 rounded-3xl font-black uppercase tracking-widest hover:bg-indigo-700 shadow-2xl shadow-indigo-200 transition-all text-xl">
            {editingServiceId ? '💾 Atualizar Atendimento' : '✅ Gravar Atendimento'}
          </button>
        </form>
      )}
          ...
          <div key={s.id} className="bg-white p-6 md:p-8 rounded-[2.5rem] border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between items-center gap-6 hover:border-indigo-500 hover:shadow-xl transition-all group overflow-hidden relative">
            <div className="flex-1 w-full text-left relative z-10">
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                ...
                >
                  <span className="text-base">📄</span> VISUALIZAR RELATÓRIO
                </button>
                <button 
            ...
            <div className="text-right border-t md:border-t-0 md:border-l border-slate-100 pt-6 md:pt-0 md:pl-10 w-full md:w-auto flex flex-row md:flex-col justify-between items-center md:items-end relative z-10">
              <div className="space-y-1">
                <p className="text-[10px] font-black text-slate-400 uppercase mb-1 tracking-widest">Total</p>
            ...
            <div className="absolute -right-4 -bottom-4 opacity-[0.03] text-9xl font-black select-none pointer-events-none group-hover:opacity-[0.06] transition-opacity uppercase">
               RM
            </div>
          </div>
        ))}
